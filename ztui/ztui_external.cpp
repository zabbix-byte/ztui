...
namespace Ztui
{
    ...
    void  ZtuiExternal::AttachAnotherWindow(std::string DestWindowName, std::string DestWindowClassName, std::function<void()> CallBack)
    {
        if (!CallBack)
            throw OSException("CallBack is empty");
    ...
    bool ZtuiExternal::PeekEndMessage()
    {
        MSG msg;
    ...
    bool ZtuiExternal::UpdateWindowData()
    {
        POINT Point{};
        RECT Rect{};
